...
type ManagedMetricStatus struct {
	...
	// conditions represent the current state of the ManagedMetric resource.
...
type ManagedMetric struct {
	...
	// spec defines the desired state of ManagedMetric
	// +required
	Spec ManagedMetricSpec `json:"spec"`

	// status defines the observed state of ManagedMetric
	// +optional
	Status ManagedMetricStatus `json:"status,omitempty,omitzero"`
}
...
// Conversion hub marker interface
func (*ManagedMetric) Hub() {}
